...
async fn get_last_txn_name(rest_client: &Client) -> Option<&'static str> {
    let txns = rest_client
        .get_transactions_bcs(None, Some(1))
        .await
        .unwrap()
        .into_inner();
    let txn_names = txns
...
async fn block_epilogue_upgrade_test() {
    ...
    swarm
        .wait_for_all_nodes_to_catchup_to_next(Duration::from_secs(MAX_WAIT_SECS))
        .await
        .unwrap();
    ...
    assert_eq!(
        get_last_txn_name(&rest_client).await,
